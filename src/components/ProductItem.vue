<template>
	<li class='product'>
		<button class='removeButton' @click='removeProduct(product._id)'>
...
export default {
	...
	methods: {
		removeProduct(productId) {
			this.$store.dispatch(actionTypes.removeProduct, productId)
		},
	},
}
</script>
...
.product {
	@apply bg-[#FFFEFB] relative rounded shadow-md relative transition-all h-[26.5rem];
	&:hover:before {
		content: '';
...
.removeButton {
	@apply absolute cursor-pointer w-8 h-8 rounded-lg right-[-0.6rem]
	top-[-0.6rem] bg-[#FF8484] opacity-0 hover:opacity-100
	hover:bg-[#e66761] transition-all flex justify-center items-center;
}
</style>
